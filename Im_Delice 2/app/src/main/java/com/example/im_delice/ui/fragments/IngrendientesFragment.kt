...
class IngrendientesFragment : Fragment() {
    ...
    override fun onResume() {
        super.onResume()
        Log.d("CategoriaFragment", "onResume llamado: recargando Ingredientes")
        // Restaurar el estado predeterminado (sin selección)
        val Filtracion = view?.findViewById<RadioGroup>(R.id.GrupoFiltracionIngredientes)
        Filtracion?.clearCheck() // Limpia la selección del RadioGroup
        cargaringrediente()

        // Opcional: cargar datos sin filtro
        val rvListadoIngrediente = view?.findViewById<RecyclerView>(R.id.rvListadoIngrediente)
